import type { Metadata } from 'next'

export const metadata: Metadata = {
  title: 'Kontakt',
  description: 'Salon Dueball Hollenstedt – Adresse, Telefon, Öffnungszeiten und Terminbuchung.',
}

const hours = [
  { day: 'Montag',     hours: 'Geschlossen', closed: true },
  { day: 'Dienstag',   hours: '08:00 – 18:00' },
  { day: 'Mittwoch',   hours: '09:00 – 19:00' },
  { day: 'Donnerstag', hours: '08:00 – 18:00' },
  { day: 'Freitag',    hours: '08:00 – 18:00' },
  { day: 'Samstag',    hours: '08:00 – 13:00' },
]

export default function KontaktPage() {
  return (
    <>
      {/* Page Hero */}
      <div className="bg-salon-warm pt-32 pb-16 text-center">
        <p className="text-salon-gold text-xs tracking-[0.3em] uppercase mb-4">Wir freuen uns auf Sie</p>
        <h1 className="font-serif text-5xl text-salon-dark mb-4">Kontakt</h1>
        <div className="divider-gold" />
      </div>

      <section className="section-padding bg-salon-cream">
        <div className="max-w-5xl mx-auto px-6">
          <div className="grid grid-cols-1 lg:grid-cols-5 gap-12">
            {/* Info column */}
            <div className="lg:col-span-2 space-y-10">
              <div>
                <h2 className="font-serif text-2xl text-salon-dark mb-6">Salon Dueball</h2>
                <address className="not-italic space-y-4">
                  <div className="flex gap-3">
                    <svg className="w-5 h-5 text-salon-gold mt-0.5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z" />
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M15 11a3 3 0 11-6 0 3 3 0 016 0z" />
                    </svg>
                    <div>
                      <p className="text-salon-dark font-medium">Adresse</p>
                      <p className="text-salon-gray text-sm mt-1">Jahnstraße 3<br />21279 Hollenstedt</p>
                    </div>
                  </div>
                  <div className="flex gap-3">
                    <svg className="w-5 h-5 text-salon-gold mt-0.5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z" />
                    </svg>
                    <div>
                      <p className="text-salon-dark font-medium">Telefon</p>
                      <a href="tel:+4941658833" className="text-salon-gray text-sm hover:text-salon-gold transition-colors mt-1 block">
                        04165 8833
                      </a>
                      <a href="tel:+4941658898" className="text-salon-gray text-sm hover:text-salon-gold transition-colors">
                        Fax: 04165 8898
                      </a>
                    </div>
                  </div>
                  <div className="flex gap-3">
                    <svg className="w-5 h-5 text-salon-gold mt-0.5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" />
                    </svg>
                    <div>
                      <p className="text-salon-dark font-medium">E-Mail</p>
                      <a href="mailto:salon.dueball@gmail.com" className="text-salon-gray text-sm hover:text-salon-gold transition-colors mt-1 block">
                        salon.dueball@gmail.com
                      </a>
                    </div>
                  </div>
                </address>
              </div>

              <div>
                <h3 className="text-xs tracking-[0.25em] uppercase text-salon-gold mb-5">Öffnungszeiten</h3>
                <ul className="space-y-2">
                  {hours.map(({ day, hours: h, closed }) => (
                    <li key={day} className="flex justify-between py-2 border-b border-salon-gray-light last:border-0">
                      <span className={`text-sm ${closed ? 'text-salon-gray/50' : 'text-salon-dark font-medium'}`}>{day}</span>
                      <span className={`text-sm ${closed ? 'text-salon-gray/40' : 'text-salon-gray'}`}>{h}</span>
                    </li>
                  ))}
                </ul>
              </div>

              {/* Payment methods */}
              <div>
                <h3 className="text-xs tracking-[0.25em] uppercase text-salon-gold mb-4">Zahlungsarten</h3>
                <div className="flex flex-wrap gap-2">
                  {['Bar', 'EC-Cash', 'Visa', 'Mastercard', 'Gutschein'].map((p) => (
                    <span key={p} className="text-xs bg-salon-warm border border-salon-gray-light px-3 py-1.5 text-salon-gray">
                      {p}
                    </span>
                  ))}
                </div>
              </div>
            </div>

            {/* Map + booking */}
            <div className="lg:col-span-3 space-y-6">
              {/* Map */}
              <div className="relative w-full h-72 bg-salon-gray-light overflow-hidden">
                <iframe
                  src="https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d2384.5!2d9.745!3d53.373!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x0%3A0x0!2zSmFobnN0cmHDn2UgMywgMjEyNzkgSG9sbGVuc3RlZHQ!5e0!3m2!1sde!2sde!4v1234567890"
                  width="100%"
                  height="100%"
                  style={{ border: 0 }}
                  allowFullScreen
                  loading="lazy"
                  referrerPolicy="no-referrer-when-downgrade"
                  title="Salon Dueball Standort"
                  className="grayscale"
                />
              </div>

              {/* Booking info */}
              <div className="bg-salon-dark text-white p-8">
                <h3 className="font-serif text-2xl mb-3">Termin buchen</h3>
                <p className="text-white/60 text-sm mb-6 leading-relaxed">
                  Wir arbeiten hauptsächlich auf Terminbasis.
                  Buchen Sie bequem online oder rufen Sie uns an.
                  Bei Absagen bitten wir um mind. 24h Vorabsage.
                </p>
                <div className="flex flex-col sm:flex-row gap-3">
                  <a
                    href="https://connect.shore.com/bookings/friseur-christina-aldag/services?locale=de"
                    target="_blank"
                    rel="noopener noreferrer"
                    className="flex-1 text-center bg-salon-gold hover:bg-salon-gold-light text-white py-3 text-sm tracking-wide transition-colors"
                  >
                    Online buchen
                  </a>
                  <a
                    href="tel:+4941658833"
                    className="flex-1 text-center border border-white/30 hover:border-white text-white py-3 text-sm tracking-wide transition-colors"
                  >
                    04165 8833 anrufen
                  </a>
                </div>
              </div>

              {/* Social */}
              <div className="bg-salon-warm p-6">
                <p className="text-salon-dark font-medium mb-4">Folgen Sie uns</p>
                <div className="flex gap-4">
                  <a
                    href="https://www.instagram.com/salon_dueball_hollenstedt/"
                    target="_blank"
                    rel="noopener noreferrer"
                    className="flex items-center gap-2 text-sm text-salon-gray hover:text-salon-gold transition-colors"
                  >
                    <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
                      <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zM12 0C8.741 0 8.333.014 7.053.072 2.695.272.273 2.69.073 7.052.014 8.333 0 8.741 0 12c0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98C8.333 23.986 8.741 24 12 24c3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98C15.668.014 15.259 0 12 0zm0 5.838a6.162 6.162 0 100 12.324 6.162 6.162 0 000-12.324zM12 16a4 4 0 110-8 4 4 0 010 8zm6.406-11.845a1.44 1.44 0 100 2.881 1.44 1.44 0 000-2.881z" />
                    </svg>
                    Instagram
                  </a>
                  <a
                    href="https://www.facebook.com/salon.dueball/"
                    target="_blank"
                    rel="noopener noreferrer"
                    className="flex items-center gap-2 text-sm text-salon-gray hover:text-salon-gold transition-colors"
                  >
                    <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
                      <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z" />
                    </svg>
                    Facebook
                  </a>
                </div>
              </div>
            </div>
          </div>
        </div>
      </section>
    </>
  )
}
